...
//
// Req/res handling + response shaping for the clients module. All raw
// Supabase/DB access lives in clients.service.js — this file validates
// input, calls the service, merges in pending approvals, builds the
...
// APPROVAL: `approvalStatus` (added by attachPendingClientApprovals below) is
...
async function attachPendingClientApprovals(formattedClients, organizationId) {
  const pending =
    await clientsService.getPendingClientApprovals(organizationId);
...
async function listClients(req, res) {
  try {
    ...
    const clients = await clientsService.getAllClients(orgId);
    const subclients = await clientsService.getAllSubclientsForOrg(orgId);
    const clientProductLinks =
      await clientsService.getClientProductLinksForOrg(orgId);
...
async function getClientById(req, res) {
  try {
    ...
    const client = await clientsService.getClientById(id, orgId);
    if (!client) {
      return res.status(404).json({ message: "Client not found" });
    }

    const subclients = await clientsService.getSubclientsForClient(id, orgId);
...
async function deleteClient(req, res) {
  try {
    ...
    try {
      await clientsService.deleteClient(id, orgId);
    } catch (error) {
      if (error.code === "23503") {
